...
const T: Record<string, any> = {
  ...
  pt: { welcome:'Bem-vindo de volta', docs:'Meus Documentos', docsDesc:'Envie e visualize seus arquivos', organizer:'Organizador Fiscal', organizerDesc:'Acompanhe o status da sua declaração', payments:'Pagamentos', messages:'Mensagens', messagesDesc:'Converse com seu contador', noBalance:'Sem saldo pendente', taxYear:'Ano Fiscal 2024', accounting:'Contabilidade', accountingDesc:'DRE, Balanço e Fornecedores' },
  es: { welcome:'Bienvenido de nuevo', docs:'Mis Documentos', docsDesc:'Suba y vea sus archivos', organizer:'Organizador Fiscal', organizerDesc:'Rastree el estado de su declaración', payments:'Pagos', messages:'Mensajes', messagesDesc:'Hable con su contador', noBalance:'Sin saldo pendiente', taxYear:'Año Fiscal 2024', accounting:'Contabilidad', accountingDesc:'Estado de resultados, Balance y Proveedores' },
  zh: { welcome:'欢迎回来', docs:'我的文件', docsDesc:'上传和查看您的文件', organizer:'税务组织器', organizerDesc:'跟踪您的申报状态', payments:'付款', messages:'消息', messagesDesc:'与您的会计师聊天', noBalance:'无未结余额', taxYear:'2024税务年度', accounting:'会计报表', accountingDesc:'损益表、资产负债表和供应商' },
  fr: { welcome:'Bon retour', docs:'Mes Documents', docsDesc:'Téléchargez et consultez vos fichiers', organizer:'Organisateur Fiscal', organizerDesc:'Suivez le statut de votre déclaration', payments:'Paiements', messages:'Messages', messagesDesc:'Discutez avec votre comptable', noBalance:'Aucun solde dû', taxYear:'Année Fiscale 2024', accounting:'Comptabilité', accountingDesc:'Compte de résultat, Bilan et Fournisseurs' },
}
...
export default async function PortalPage() {
  ...
  const lang      = (client?.language || 'pt') as string
  const t         = T[lang] || T.pt
  const firstName = client?.name?.split(' ')[0] || user?.email?.split('@')[0] || 'Cliente'
  const balance   = client?.balance || 0
  const ehEmpresa = client?.type === 'business'
  ...
  const quickLinks = [
    ...
    // Contabilidade: só para conta empresarial, que é quem tem bookkeeping
    ...(ehEmpresa ? [{ href:'/portal/reports', icon:'📊', label:t.accounting, desc:t.accountingDesc }] : []),
    ...
    <div>
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ color:'#6a7a9a', fontSize:14, margin:0 }}>
          {ehEmpresa ? '🏢' : '👤'} {client?.name} · {t.taxYear}
        </p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:14, marginBottom:28 }}>
        {quickLinks.map(l => (
          <Link key={l.href} href={l.href} style={{ textDecoration:'none' }}>
            <div style={{ background:(l as any).highlight ? 'linear-gradient(135deg,#2D3278,#1a1f5e)' : '#fff', borderRadius:14, padding:'18px 20px', border:(l as any).highlight ? 'none' : '1px solid #e2e8f4', cursor:'pointer', display:'flex', alignItems:'center', gap:14 }}>
              <div style={{ width:46, height:46, borderRadius:12, background:(l as any).highlight ? 'rgba(255,255,255,0.15)' : '#f0f4fa', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22, flexShrink:0 }}>
                {l.icon}
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:14, fontWeight:700, color:(l as any).highlight ? '#fff' : '#1a2a3a' }}>{l.label}</div>
                <div style={{ fontSize:12, color:(l as any).highlight ? 'rgba(255,255,255,0.7)' : '#6a7a9a', marginTop:2 }}>{l.desc}</div>
              </div>
              <span style={{ color:(l as any).highlight ? 'rgba(255,255,255,0.6)' : '#9aaab0', fontSize:16 }}>→</span>
            </div>
          </Link>
        ))}
      </div>
      ...
      <div style={{ background:'#fff', borderRadius:14, padding:20, border:'1px solid #e2e8f4' }}>
        ...
        {(!docs || docs.length === 0) ? (
          <div style={{ textAlign:'center', padding:'24px 0', color:'#9aaab0' }}>
            ...
            <Link href="/portal/documents" style={{ background:'#2D3278', color:'#fff', padding:'8px 18px', borderRadius:8, fontSize:12, fontWeight:700, textDecoration:'none' }}>
