// POST /api/clients/profile â€” equipe edita dados de contato/status do cliente
...
export async function POST(req: NextRequest) {
  ...
  if (!clientId || !fields || typeof fields !== 'object') {
    return NextResponse.json({ error: 'clientId e fields obrigatÃ³rios' }, { status: 400 })
  }

  // Filtra sÃ³ campos permitidos
  const patch: Record<string, unknown> = {}
  for (const [k, v] of Object.entries(fields)) {
    if (EDITABLE.has(k)) patch[k] = v
  }
  if (Object.keys(patch).length === 0) {
    return NextResponse.json({ error: 'Nenhum campo editÃ¡vel informado' }, { status: 400 })
  }
  ...
  if (level === 'junior') {
    if (!reason?.trim()) return NextResponse.json({ error: 'Motivo obrigatÃ³rio (nÃ­vel junior)' }, { status: 403 })
    approvedBy = await validateManagerPin(managerPin ?? '')
    if (!approvedBy) return NextResponse.json({ error: 'PIN de manager invÃ¡lido' }, { status: 403 })
  } else if (isActiveChange && !reason?.trim()) {
    // Ativar/desativar sempre exige motivo, mesmo owner/manager
    return NextResponse.json({ error: 'Motivo obrigatÃ³rio para ativar/desativar cliente' }, { status: 400 })
  }

  const db = serviceDb()
  const { data: current } = await db.from('clients').select('*').eq('id', clientId).single()
  if (!current) return NextResponse.json({ error: 'Cliente nÃ£o encontrado' }, { status: 404 })
